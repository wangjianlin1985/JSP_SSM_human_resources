...
public class EmployeeController extends BaseController {
	...
	public void add(@Validated Employee employee, BindingResult br,
		...
		try {
			employee.setEmpPhoto(this.handlePhotoUpload(request, "empPhotoFile"));
		...
		employee.setRegTime(sdf.format(new java.util.Date()));
	...
	public void list(String employeeNo,@ModelAttribute("departmentObj") Department departmentObj,String positionName,String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, departmentObj, positionName, name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, departmentObj, positionName, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String employeeNo,@ModelAttribute("departmentObj") Department departmentObj,String positionName,String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, departmentObj, positionName, name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, departmentObj, positionName, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = employeeService.getTotalPage();
	...
	public void update(@Validated Employee employee, BindingResult br,
		...
		String empPhotoFileName = this.handlePhotoUpload(request, "empPhotoFile");
		if(!empPhotoFileName.equals("upload/NoImage.jpg"))employee.setEmpPhoto(empPhotoFileName); 
	...
	public void OutToExcel(String employeeNo,@ModelAttribute("departmentObj") Department departmentObj,String positionName,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(employeeNo == null) employeeNo = "";
        if(positionName == null) positionName = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Employee> employeeList = employeeService.queryEmployee(employeeNo,departmentObj,positionName,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Employee信息记录"; 
        String[] headers = { "员工编号","所在部门","职位名称","姓名","性别","出生日期","员工照片","联系电话","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<employeeList.size();i++) {
        	Employee employee = employeeList.get(i); 
        	dataset.add(new String[]{employee.getEmployeeNo(),employee.getDepartmentObj().getDepartmentName(),employee.getPositionName(),employee.getName(),employee.getGender(),employee.getBirthDate(),employee.getEmpPhoto(),employee.getTelephone(),employee.getRegTime()});
        }
        /*
